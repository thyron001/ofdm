"""
=====================================================================================
 SFBC — Diversidad en TRANSMISIÓN (código de Alamouti) — solo para LEER y entender
=====================================================================================
 ...
 SFBC (Space-Frequency Block Coding) usa 2 antenas TRANSMISORAS. Trabaja por PARES de
 subportadoras adyacentes (k, k+1). Para cada par se toman dos símbolos (s0, s1):
        Antena 1:  subportadora k → s0 ,  subportadora k+1 → s1
        Antena 2:  subportadora k → -s1*,  subportadora k+1 → s0*
 Cada antena se escala por 1/√2 para repartir la potencia total. Como las dos
 subportadoras del par son adyacentes, el canal es casi igual en ambas (H[k] ≈ H[k+1]),
 y el combinador ortogonal de Alamouti recupera s0 y s1 SIN interferencia, ganando
 diversidad (2x1 → orden 2, 2x2 → orden 4) sin conocer el canal en el transmisor.

 Flujo:
   bits → QAM → pares (s0,s1) → Alamouti (2 antenas) → IFFT+CP → [2 canales + ruido]
        → FFT → combinador Alamouti → rearmar pares → QAM → bits
=====================================================================================
"""
...
#  Entra el bloque de símbolos QAM y salen dos vectores: s0 (símbolos en posición par,
#  van en la subportadora k) y s1 (posición impar, van en la subportadora k+1).
# _____________________________________________________________________________________
def separar_pares(simbolos):
    s0 = simbolos[0::2]   # Primer símbolo de cada par → subportadora k
    s1 = simbolos[1::2]   # Segundo símbolo de cada par → subportadora k+1
    return s0, s1


# _____________________________________________________________________________________
#  CODIFICAR ALAMOUTI (EL PASO CLAVE EN EL TX)                               Transmisión
#  Entran los pares (s0, s1) y salen las DOS rejillas de subportadoras, una por antena:
#       Antena 1:  k → s0 ,   k+1 → s1
#       Antena 2:  k → -s1*,  k+1 → s0*
#  Ambas escaladas por 1/√2 para repartir la potencia total entre las dos antenas.
# _____________________________________________________________________________________
def codificar_alamouti(s0, s1, n_sc, esc=ESC):
    n_par = n_sc // 2                          # Nº de pares de subportadoras
...
#  y salen sus n_sc subportadoras (en frecuencia): se descarta el CP y se aplica la FFT.
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  COMBINADOR ALAMOUTI (EL PASO CLAVE EN EL RX)                               Recepción
#  Entra una lista con una tupla (r0, r1, h1, h2) por cada antena receptora, donde r0,r1
#  son lo recibido en el par (k, k+1) y h1,h2 los canales de las antenas TX1, TX2 en ese
#  par. Salen las estimaciones de s0 y s1, separadas sin interferencia:
#       s0_est = Σ (conj(h1)·r0 + h2·conj(r1)) / Σ (|h1|²+|h2|²)
#       s1_est = Σ (conj(h1)·r1 - h2·conj(r0)) / Σ (|h1|²+|h2|²)
#  El /esc final deshace el reparto de potencia 1/√2 (devuelve la escala unitaria).
# _____________________________________________________________________________________
def combinar_alamouti(recibidos, esc=ESC):
    ...
    for r0, r1, h1, h2 in recibidos:
        num0 += np.conj(h1) * r0 + h2 * np.conj(r1)
...
# =====================================================================================
# ||    FLUJO COMPLETO — cómo se encadenan los pasos (un símbolo, 2 TX × n_rx RX)     ||
# =====================================================================================

# _____________________________________________________________________________________
#  CADENA SFBC (2 antenas TX → n_rx antenas RX)                           Todo el flujo
#  Entran símbolos QAM de datos (cantidad par) y salen recuperados. Cada antena RX recibe
#  la suma de las 2 antenas TX (cada una por su propio canal) y se decodifica con el
#  combinador de Alamouti.
# _____________________________________________________________________________________
def cadena_sfbc(simbolos_datos, n_sc, n_fft, n_cp, fs, snr_db, rng, n_rx):
    n_par = n_sc // 2

    # --- TRANSMISIÓN ---
    s0, s1 = separar_pares(simbolos_datos)                   # Pares (s0, s1)
    X1, X2 = codificar_alamouti(s0, s1, n_sc)               # ← PASO CLAVE: 2 rejillas Alamouti
    _, indices_fft = mapeo_sc_a_fft(X1, n_fft)              # Qué bins de la FFT se usan

    # --- CANAL + RECEPCIÓN por antena RX: ve la suma de TX1 y TX2 por canales distintos ---
    recibidos = []
    for _ in range(n_rx):
    ...
    # --- COMBINACIÓN ---
    s0_est, s1_est = combinar_alamouti(recibidos)   # ← PASO CLAVE: separa s0 y s1 sin interferencia
