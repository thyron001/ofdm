...
=====================================================================================
 Este archivo NO ejecuta nada (no hay servidor, ni Monte Carlo, ni imágenes). Contiene
 SOLO el código de los BLOQUES del diagrama que se muestra en la interfaz, en el mismo
 orden en que aparecen:

     Fuente → Bits → Demux (N señales) → Coding & Modulation POR SEÑAL (PARC)
        → Mapeo a antenas + IFFT/CP → Canal MIMO H + AWGN → Quita CP + FFT
        → SIC (detectar → decodificar → restar) → Remux → Fuente

 Cada bloque del diagrama se corresponde con una función de este archivo:
   · Fuente → Bits ................. texto_a_bits            (y bits_a_texto en el RX)
   · Demux (N señales) ............ demux_bits
   · Coding & Modulation POR SEÑAL  perfil_parc + modular_senales (usa mapear_qpsk/16qam/64qam)
   · Mapeo a antenas + IFFT/CP .... mapear_a_antenas + mapeo_sc_a_fft + modulacion_ofdm
   · Canal MIMO H + AWGN .......... generar_canal_mimo (usa generar_canal_pedestrian_a) + agregar_ruido_awgn
   · Quita CP + FFT ............... demodulacion_ofdm
   · SIC .......................... sic_rx (usa detector_mmse_etapa y demapear_qpsk/16qam/64qam)
   · Remux → Fuente ............... remux_bits (y bits_a_texto)
 El flujo completo que encadena todos los bloques es cadena_mimo_sm.

 Idea de la práctica: con N antenas TX y N antenas RX (config SIMÉTRICA, N_T = N_R = N)
 se transmiten N SEÑALES INDEPENDIENTES a la vez, una por antena. La fuente de bits puede
 ser una imagen (lena por defecto) o un texto. El esquema de multiplexación es POR ANTENA
 y POR SUBPORTADORA (OFDM): en la subportadora k viajan N símbolos QAM a la vez (uno por
 antena) — las señales no se separan en frecuencia ni en tiempo, sino en el ESPACIO,
 usando la matriz de canal H[k] de esa subportadora. Así la tasa se multiplica por N y el
 tiempo de envío se divide ≈ N; el precio es que cada antena RX recibe la SUMA de las N
 señales (se interfieren mutuamente), y a más antenas, más interferencia.

 TX MULTI-CODEWORD (diapo 204): cada señal tiene su PROPIO "Coding & modulation", lo que
 permite darle DIFERENTE ROBUSTEZ (PARC, diapo 208): la 1ª señal en decodificarse sufre la
 interferencia de todas las demás → se le da la modulación más robusta (QPSK); las últimas,
 ya casi sin interferencia, usan modulaciones más agresivas (16/64-QAM).

 RX SIC — Successive Interference Cancellation (diapos 206-207): elimina la interferencia
 POR ETAPAS. Detecta la 1ª señal (MMSE), la DECODIFICA (decisión dura), la RE-CODIFICA y la
 RESTA de lo recibido por todas las antenas; la 2ª señal se decodifica con SIR mejorada, se
 resta… y así hasta la señal N.

 Modelo por subportadora:  r̄[k] = H[k]·s̄[k] + n̄[k]  (H[k] de N×N, cada antena TX a 1/√N).
=====================================================================================
"""
...
#  BLOQUE "Fuente → Bits"                                                    Transmisión
#  La fuente puede ser una imagen (sus píxeles se desempaquetan a bits) o un TEXTO: cada
#  carácter UTF-8 se convierte en su byte y cada byte en 8 bits (MSB primero). Aquí se
#  muestra la de texto; la de imagen vive en app.py (imagen_a_bits).
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "Coding & Modulation" — MAPEO QPSK                                 Transmisión
...
# _____________________________________________________________________________________
#  BLOQUE "Coding & Modulation" — MAPEO 16-QAM                               Transmisión
...
# _____________________________________________________________________________________
#  BLOQUE "Coding & Modulation" — MAPEO 64-QAM                               Transmisión
...
# _____________________________________________________________________________________
#  BLOQUE "Coding & Modulation" — PERFIL PARC (Per-Antenna Rate Control)     Transmisión
#  Entra la modulación "objetivo" (la de la señal MENOS interferida) y el nº de señales N,
...
# _____________________________________________________________________________________
#  BLOQUE "Demux (N señales)"                                                Transmisión
...
#  consecutivos. Los datos vienen de la MISMA fuente (imagen o texto) y se reparten en N
#  señales distintas ANTES de la modulación (TX multi-codeword, diapo 205).
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "Coding & Modulation POR SEÑAL" (EL PASO CLAVE DEL TX, diapo 204)  Transmisión
#  Entran los bits de cada señal y su perfil PARC, y sale la matriz S (N × N_SC): la FILA i
#  es la señal i (una por ANTENA) y la COLUMNA k su símbolo en la SUBPORTADORA k. Esta
#  matriz ES el esquema de multiplexación por antena y por subportadora. Cada señal es
#  INDEPENDIENTE y se modula con SU PROPIA modulación (multi-codeword): la tasa se
#  multiplica por N (a diferencia de la diversidad, que envía la misma info redundante).
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "Mapeo a antenas"                                                  Transmisión
#  Entra la matriz de señales S y sale X = S/√N: la señal i se asigna a la antena i, con
#  reparto de potencia 1/√N para que la potencia TOTAL radiada sea constante (la misma que
#  un SISO a plena potencia). Ese reparto cuesta ~10·log10(N) dB por señal: parte del
#  precio de multiplicar la tasa.
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "IFFT+CP" — MAPEO SUBPORTADORAS → BINS DE LA FFT                   Transmisión
#  Entra una rejilla de n_sc subportadoras y sale el vector de n_fft puntos en banda base
#  (DC vacía, centrado en ±frecuencia) y los índices FFT usados.
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "IFFT+CP" — MODULACIÓN OFDM (IFFT + PREFIJO CÍCLICO)               Transmisión
#  Entra la rejilla de una antena y sale su señal OFDM en el tiempo con el prefijo cíclico
#  (CP), junto con los índices FFT usados. Se aplica a cada antena por separado: N antenas
#  → N moduladores OFDM en paralelo, uno por señal.
# _____________________________________________________________________________________
...
#  enlaces en total. La baja correlación entre enlaces es lo que permite separar las
#  señales (diapo 181). Devuelve un tensor H de forma (N, N, N_SC): para la subportadora k,
...
# _____________________________________________________________________________________
#  BLOQUE "Quita CP + FFT" — DEMODULACIÓN OFDM                                Recepción
#  Entra la señal recibida por UNA antena — que es la SUMA de las N señales TX, cada una a
#  través de su propio canal: AHÍ está la interferencia entre señales — y salen sus n_sc
#  subportadoras. Se aplica a cada una de las N antenas RX.
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "SIC" — DETECTOR MMSE DE UNA ETAPA (el "detectar" del SIC)          Recepción
...
#  El término (N0/Es)·I frena el realce de ruido. OJO: con la normalización OFDM el cargado
#  N0/Es vale (potencia_recibida)·(N_SC/N_FFT)/SNR, no simplemente 1/SNR (si se usara 1/SNR
#  el MMSE quedaría mal calibrado).
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "SIC" — CANCELACIÓN SUCESIVA DE INTERFERENCIA (EL PASO CLAVE DEL RX) Recepción
#  Implementa el receptor de las diapositivas 206-207. Entra lo recibido R (N × N_SC), el
#  canal efectivo Hg (N × N × N_SC, ya con el 1/√N) y el perfil PARC. Para cada señal
#  i = 0 … N-1 (¡en orden de robustez!):
...
def sic_rx(R, Hg, perfil, carga_ruido):
    ...
    for i in range(n_ant):                                    # ← Etapas del SIC (diapo 207)
        ...
        bits_i = mod["demapear"](s_est)                       # 2. "Decoding" (decisión dura)
...
# _____________________________________________________________________________________
#  BLOQUE "Remux → Fuente" — REMULTIPLEXAR LOS BITS                           Recepción
#  Entran los bits decodificados de cada señal y sale el bloque en el orden original (el
#  inverso exacto de demux_bits), listo para rearmar la imagen o el texto.
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "Remux → Fuente" — BITS → TEXTO                                     Recepción
#  Reagrupa los bits recibidos en bytes y los decodifica como UTF-8. Un byte corrupto por
#  errores de bit se muestra como "�": con la fuente de texto, la calidad del enlace se VE
#  directamente en el mensaje recuperado.
# _____________________________________________________________________________________
...
# _____________________________________________________________________________________
#  BLOQUE "SIC" (decodificación) — DEMAPEO QPSK                               Recepción
...
# _____________________________________________________________________________________
#  BLOQUE "SIC" (decodificación) — DEMAPEO 16-QAM                             Recepción
...
# _____________________________________________________________________________________
#  BLOQUE "SIC" (decodificación) — DEMAPEO 64-QAM                             Recepción
...
# =====================================================================================
# ||    FLUJO COMPLETO — cómo se encadenan los BLOQUES (un uso de canal, N × N)       ||
# =====================================================================================

# _____________________________________________________________________________________
#  CADENA MULTIPLEXACIÓN ESPACIAL MULTI-CODEWORD + SIC                    Todo el flujo
#  Entra el bloque de bits de UN uso de canal (n_sc·Σbps bits de la imagen o del texto) y
#  salen los bits recuperados en el mismo orden. Encadena, en este orden, los bloques del
#  diagrama de la interfaz: Demux → Coding&Mod por señal → Mapeo a antenas + IFFT/CP →
#  Canal MIMO + AWGN → Quita CP + FFT → SIC → Remux. Las N señales viajan SIMULTÁNEAMENTE
...
# _____________________________________________________________________________________
def cadena_mimo_sm(bits_uso, n_sc, n_fft, n_cp, fs, snr_db, rng, n_ant, perfil):
    # --- TRANSMISIÓN: Demux → Coding & Modulation POR SEÑAL → Mapeo a antenas ---
    flujos_bits = demux_bits(bits_uso, n_sc, perfil)         # Demux (N señales)
    S = modular_senales(flujos_bits, perfil, n_sc)           # Coding & Mod POR SEÑAL (PARC)
    X = mapear_a_antenas(S, n_ant)                           # Mapeo a antenas (×1/√N)
    ...
    for r in range(n_ant):
        ...
        R[r] = demodulacion_ofdm(senal, indices_fft, n_fft, n_cp, n_sc)  # Quita CP + FFT

    # --- RECEPCIÓN: SIC (diapo 207) ---
    ...
    return remux_bits(bits_por_senal)                        # Remux → bits en el orden original
